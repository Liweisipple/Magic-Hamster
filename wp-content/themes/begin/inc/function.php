...
function article_catalog($content) {
	...
	if(preg_match_all($r, $content, $matches)) {
		...
		$content = "
			\n<div id=\"log-box\">
				<div id=\"catalog\"><ul id=\"catalog-ul\">\n" . $ul_li . "</ul><span class=\"log-zd\"><span class=\"log-close\"><a title=\"" . sprintf(__( '隐藏目录', 'begin' )) . "\"><i class=\"fa fa-times\"></i><strong>" . sprintf(__( '目录', 'begin' )) . "</strong></a></span></span></div>
			</div>\n" . $content;
	}
	return $content;
}
...
function reply_read($atts, $content=null) {
	...
	<div class="reply-read">
		<div class="reply-ts">
			<div class="read-sm"><i class="fa fa-exclamation-circle"></i>' . sprintf(__( '此处为隐藏的内容！', 'begin' )) . '</div>
			<div class="read-sm"><i class="fa fa-spinner"></i>' . sprintf(__( '发表评论并刷新，才能查看', 'begin' )) . '</div>
		</div>
		<div class="read-pl"><a href="#respond">' . sprintf(__( '发表评论', 'begin' )) . '</a></div>
		<div class="clear"></div>
    </div>'), $atts));
	$email = null;
...
function login_to_read($atts, $content = null) {
	...
	<div class="reply-read">
		<div class="reply-ts">
			<div class="read-sm"><i class="fa fa-exclamation-circle"></i>' . sprintf(__( '此处为隐藏的内容！', 'begin' )) . '</div>
			<div class="read-sm"><i class="fa fa-sign-in"></i>' . sprintf(__( '登录后才能查看！', 'begin' )) . '</div>
		</div>
		<div class="read-pl"><a href="#login" class="flatbtn" id="login-see" >' . sprintf(__( '登录', 'begin' )) . '</a></div>
		<div class="clear"></div>
	</div>'), $atts));
	if (is_user_logged_in()) {
...
if(isset($_POST['secret_key']) && $_POST['secret_key']==$key){
	return '<div class="secret-password"><i class="fa fa-check-square"></i>加密的内容：<br />'.do_shortcode( $content ).'</div>';
		...
		<form class="post-password-form" action="'.get_permalink().'" method="post">
			<div class="post-secret"><i class="fa fa-exclamation-circle"></i>' . sprintf(__( '输入密码查看加密内容：', 'begin' )) . '</div>
			<p>
...
function begin_crumbs() {
		if (is_home()) {
			if (zm_get_option('bulletin')) {
				...
				echo '<i class="fa fa-home"></i>' . sprintf(__( '现在位置', 'begin' )) . '<i class="fa fa-angle-right"></i>' . sprintf(__( '首页', 'begin' )) . '';
			}
		}
		...
		if (!is_home() && !is_front_page()) {
			...
			echo home_url('/');
			echo '">';
			echo '<i class="fa fa-home"></i>' . sprintf(__( '首页', 'begin' )) . '';
			echo "</a>";
		}
	...
	elseif (is_404()) {echo '<i class="fa fa-angle-right"></i>';echo'' . sprintf(__( '亲，你迷路了！', 'begin' )) . ''; echo'';}
	elseif (is_search()) {
		echo '<i class="fa fa-angle-right"></i>' . sprintf(__( '搜索', 'begin' )) . ' ';
...
function begin_entry_meta() {
	...
	echo '<ul class="single-meta">';
		...
		echo '<li class="print"><a href="javascript:printme()" target="_self" title="' . sprintf(__( '打印', 'begin' )) . '"><i class="fa fa-print"></i></a></li>';
		...
		echo '<li class="r-hide"><a href="javascript:pr()" title="' . sprintf(__( '侧边栏', 'begin' )) . '"><i class="fa fa-caret-left"></i> <i class="fa fa-caret-right"></i></a></li>';
	echo '</ul>';
...
function begin_single_meta() {
	echo '<div class="begin-single-meta">';
		...
		echo '<span class="s-hide r-hide"><a href="javascript:pr()" title="' . sprintf(__( '侧边栏', 'begin' )) . '"><i class="fa fa-caret-left"></i> <i class="fa fa-caret-right"></i></a></span>';
	echo '</div>';
}
...
function button_a($atts, $content = null) {
	return '<div class="down"><a class="d-popup" title="下载链接" href="#button_file"><i class="fa fa-download"></i>下载地址</a><div class="clear"></div></div>';
}

// 自定义按钮
function button_b($atts, $content = null) {
	return '<div class="down"><a class="d-popup" title="下载链接" href="#button_file"><i class="fa fa-download"></i>'.$content.'</a><div class="clear"></div></div>';
}
...
function show_more($atts, $content = null) {
	return '<span class="show-more" title="' . (__( '文字折叠', 'begin' )) . '"><span><i class="fa fa-plus-square"></i>' . sprintf(__( '展开', 'begin' )) . '</span></span>';
}
...
if (zm_get_option('vip')) {
	function get_author_class($comment_author_email,$user_id){
		...
		if($author_count>=0 && $author_count<2)
			echo '<a class="vip vip0" title="评论达人 VIP.0"><i class="fa fa-heart-o"></i><span class="lv">0</span></a>';
		else if($author_count>=2 && $author_count<5)
			echo '<a class="vip vip1" title="评论达人 VIP.1"><i class="fa fa-heart"></i><span class="lv">1</span></a>';
		else if($author_count>=5 && $author_count<10)
			echo '<a class="vip vip2" title="评论达人 VIP.2"><i class="fa fa-heart"></i><span class="lv">2</span></a>';
		else if($author_count>=10 && $author_count<20)
			echo '<a class="vip vip3" title="评论达人 VIP.3"><i class="fa fa-heart"></i><span class="lv">3</span></a>';
		else if($author_count>=20 && $author_count<50)
			echo '<a class="vip vip4" title="评论达人 VIP.4"><i class="fa fa-heart"></i><span class="lv">4</span></a>';
		else if($author_count>=50 && $author_count<100)
			echo '<a class="vip vip5" title="评论达人 VIP.5"><i class="fa fa-heart"></i><span class="lv">5</span></a>';
		else if($author_count>=100 && $author_count<200)
			echo '<a class="vip vip6" title="评论达人 VIP.6"><i class="fa fa-heart"></i><span class="lv">6</span></a>';
		else if($author_count>=200 && $author_count<300)
			echo '<a class="vip vip7" title="评论达人 VIP.7"><i class="fa fa-heart"></i><span class="lv">7</span></a>';
		else if($author_count>=300 && $author_count<400)
			echo '<a class="vip vip8" title="评论达人 VIP.8"><i class="fa fa-heart"></i><span class="lv">8</span></a>';
		else if($author_count>=400)
			echo '<a class="vip vip9" title="评论达人 VIP.9"><i class="fa fa-heart"></i><span class="lv">9</span></a>';
	}
}
	...
	foreach($results as $result) {
		...
		<div class="lx7">
			<div class="top-author">
				...
				<div class="top-comment">最后发表于' . human_time_diff(strtotime($result->last_commented_date)) . '前</div>
			</div>
		</div>';
	}
